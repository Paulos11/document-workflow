...
export const getDocumentById = (id: string): AppDocument | undefined => {
  return MOCK_DOCUMENTS.find(doc => doc.id === id);
};

export const getDocumentsByStatus = (status?: string): AppDocument[] => {
  if (!status || status === 'All Status') return MOCK_DOCUMENTS;
  return MOCK_DOCUMENTS.filter(doc => doc.status === status);
};

export const getStatusCounts = (documents: AppDocument[]): Record<string, number> => {
