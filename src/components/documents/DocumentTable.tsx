...
type StatusFilterType = DocumentStatus | "All Status";
...
export default function DocumentTable({ documents }: DocumentTableProps) {
  ...
  const statusFilters: StatusFilterType[] = ["All Status", "Submitted", "In Review", "Changes Requested", "Approved"];
  ...
  const handleStatusFilterChange = (filter: StatusFilterType): void => {
  ...
  const handleView = (doc: AppDocument): void => {
    navigate(`/document/${doc.id}`);
  };

  const handleEdit = (doc: AppDocument): void => {
    navigate(`/document/${doc.id}`);
  };
